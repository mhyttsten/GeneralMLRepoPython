...
def my_model_fn(
    ...
    logits = tf.layers.dense(h2, 3, name='MH_LOGITS')
    logits_single_handle = tf.identity(logits, name='MH_LOGITS_SINGULAR')
...
print("---------------------------------------------")
print("Now trying SavedModel")

PATH_SAVEDMODEL_BASE = PATH + os.sep + "savedmodel"
classifier.export_savedmodel(
    PATH_SAVEDMODEL_BASE,
    tf.estimator.export.build_raw_serving_input_receiver_fn(new_input_fn()[0]))
print("Done exporting savedmodel")

# Saved model directories are in timestamp order
# So take the 2nd from the last (last is its underlying variables directory)
# Element 0: Absolute path, 2: Subdirectories contained, 3: Files contained
savedmodel_dirs = []
for x in os.walk(PATH_SAVEDMODEL_BASE):
    savedmodel_dirs.append(x)
savedmodel_dir = savedmodel_dirs[len(savedmodel_dirs)-2][0]
print("SavedModel directory: {}".format(savedmodel_dir))

print("Does it contain a savedmodel: {}".format(
    tf.saved_model.loader.maybe_saved_model_directory(savedmodel_dir)))
