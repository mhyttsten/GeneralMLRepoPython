...
The agent uses a single set of weights, within which each value is an estimate of the value
of the return from choosing a particular arm given a bandit.
We use a policy gradient method to update the agent by moving the value for the selected
action toward the recieved reward.
'''
class Agent():
...
We will train our agent by getting a state from the environment,
take an action, and recieve a reward.
Using these three things, we can know how to properly update our network
in order to more often choose actions given states that will yield the
highest rewards over time.
'''
    ...
    while i < total_episodes:
        ...
        if i % 500 == 0:
            print("Mean reward for each of the {} bandits: {}"
                  .format(str(cBandit.num_bandits), str(np.mean(total_reward, axis=1))))
        i += 1

for a in range(cBandit.num_bandits):
    print("The agent thinks action {} for bandit {} is the most promising...."
          .format(str(np.argmax(ww[a]) + 1), str(a + 1)))
